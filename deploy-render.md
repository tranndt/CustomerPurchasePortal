...
### 2. Using Secret Files in Render (Recommended)

Render's Secret Files feature allows you to securely store complete configuration files:

1. **Prepare your secret files locally**
   * Create a `secrets.env` file with environment variables
   * Create a `watsonx.json` file with API credentials

2. **Add Secret Files in Render dashboard**
   * Go to the Environment tab of your service
   * Select "Secret Files" section
   * For each file:
     * File name: `secrets.env` or `watsonx.json`
     * Mount path: `/etc/secrets/secrets.env` or `/etc/secrets/watsonx.json`
     * Content: Copy from your local template and add real values

3. **Our application will automatically load these files at startup**

### 3. Alternative: Configure Environment Variables in Render
...
See [RENDER_SECRET_FILES.md](docs_backup/RENDER_SECRET_FILES.md) for more detailed setup instructions.
