      ...
      {activeTab === 'all' && (
        <div className="tab-content">
          <div className="section-header">
            <h2>All Orders</h2>
            <p>Complete order history and management</p>
          </div>
                  ...
                  <tr key={order.id}>
                    ...
                    <td>
                      <div className="customer-info">
                        ...
                        <div className="username">@{order.customer_username}</div>
                      </div>
                    </td>
                    <td>
                      <div className="product-info">
                        <div>{order.product_name}</div>
                        <div className="category">{order.product_category}</div>
                      </div>
                    </td>
                    ...
                    <td>
                      {order.processed_by ? (
                        <div className="processed-info">
                          <div>{order.processed_by}</div>
                          <div className="timestamp">
                            {order.processed_at ? new Date(order.processed_at).toLocaleDateString() : ''}
                          </div>
                        </div>
                      ) : (
                    ...
                    <td className="transaction-id">{order.transaction_id}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeTab === 'inventory' && (
        <div className="tab-content">
          <div className="section-header">
            ...
            <p>Monitor stock levels and pending orders</p>
          </div>

          <div className="inventory-grid">
            {inventory.map(item => (
              <div key={item.id} className={`inventory-card ${item.is_out_of_stock ? 'out-of-stock' : item.is_low_stock ? 'low-stock' : ''}`}>
                <h3>{item.name}</h3>
                <div className="inventory-details">
                  ...
                  <div className="detail-row">
                    ...
                    <span className={item.is_out_of_stock ? 'stock-critical' : item.is_low_stock ? 'stock-warning' : 'stock-ok'}>
                      {item.current_stock} units
                    </span>
                  </div>
                  <div className="detail-row">
                    <span>Pending Orders:</span>
                    <span>{item.pending_orders} units</span>
                  </div>
                  <div className="detail-row">
                    <span>Available After Pending:</span>
                    <span className={item.available_after_pending <= 0 ? 'stock-critical' : item.available_after_pending <= 5 ? 'stock-warning' : 'stock-ok'}>
